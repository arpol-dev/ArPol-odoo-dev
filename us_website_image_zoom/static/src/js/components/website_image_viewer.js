...
export class ImageViewer extends Dialog {
    setup() {
        super.setup();
        ...
        this.images.forEach((image, index) => {
            const src = image.thumbnailSrc;
            this.isDuplicate = this.images.some((otherImage, otherIndex) => index !== otherIndex && otherImage.thumbnailSrc === src);
        });
    ...
    onGlobalClick(ev) {
        ...
        if (ev.target.classList.contains('o_website_image_viewer_void') && !this.isDragging) {
            ev.stopPropagation();
            ev.preventDefault();
            this.data.close();
        } else {
    ...
    zoomIn(step=undefined) {
        this.state.imageScale = Math.min(1.5, this.state.imageScale + (step || ZOOM_STEP));
    }
...
delete ImageViewer.props.slots;
ImageViewer.template = "us_website_image_zoom.ImageViewer";

export class ImageViewerWrapper extends Component {
    setup() {
        this.dialogs = useWowlService('dialog');

        onRendered(() => {
            this.dialogs.add(ImageViewer, this.props);
        });
    }
}
ImageViewerWrapper.template = xml``;
